...
def add_text(text_lines, style="Normal", left=30, top=None, width=650, height=None):
    ...
    if top is None:
        top = current_top
    if height is None:
        height = max(24, len(text_lines) * 20 + 10)
    ...
    current_top = top + height + 25

def add_math(math_xml, left=30, top=None, width=200, height=30, advance=True, dtop=50):
    global current_top
    if top is None:
        top = current_top
...
add_math('''<ml:define>
    ...
    <ml:real>1</ml:real>
</ml:define>''', left=30, top=30, width=80, height=12, advance=True, dtop=35)
...
add_text([
    ...
    "  3. Separador T1 (Flash / Condensador): Separa gases incondensables por cabeza (F5) y líquidos por fondo (F4).",
    "     Especificación de claves: LK = Acetileno (0.01 en fondo), HK = Acetaldehído (0.01 en tope).",
    "  4. Divisor de gas S1: Divide la corriente F5 en purga gaseosa F6 (beta_1 = 0.20) y reciclo F7 (1 - beta_1).",
    "  5. Torre T2: Primera destilación. Separa mezcla orgánica por cabeza (F9) y agua por fondo (F8).",
    "     Especificación de claves: LK = Etanol (0.01 en fondo), HK = Agua (0.01 en tope).",
    "  6. Divisor de agua S2: En el caso base purga el fondo F8 hacia F15 (beta_2 = 1.0, F16 = 0).",
    "  7. Torre T3: Purificación de subproducto. Separa acetaldehído por cabeza (F11) y etanol crudo por fondo (F10).",
    "     Especificación de claves: LK = Acetaldehído (0.01 en fondo), HK = Etanol (0.01 en tope).",
    "  8. Torre T4: Purificación final. Separa etanol producto por cabeza (F13) y residuo acuoso por fondo (F12).",
    "     Especificación de claves: LK = Etanol (0.01 en fondo), HK = Agua (0.01 en tope)."
...
</ml:define>''', left=30, top=top_params, width=100, height=95, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve">PM</ml:id>
</ml:eval>''', left=150, top=top_params, width=120, height=95, advance=False)

# Matriz estequiométrica nu (6x2) definida en orden COLUMN-MAJOR para Mathcad XML:
# Columna 1 (Reaccion 1: C2H4 + H2O -> C2H5OH):  [0, -1,  0, 0, 1, -1]^T
# Columna 2 (Reaccion 2: C2H2 + H2O -> CH3CHO):   [0,  0, -1, 1, 0, -1]^T
add_math('''<ml:define>
...
</ml:define>''', left=300, top=top_params, width=120, height=95, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve">&#x03bd;</ml:id>
</ml:eval>''', left=440, top=top_params, width=140, height=95, advance=True, dtop=125)
...
</ml:define>''', left=430, top=top_specs1, width=80, height=12, advance=True, dtop=45)

# Split specifications LK and HK for all 4 towers: LK1..LK4 = 0.01, HK1..HK4 = 0.01
top_splits = current_top + 10
add_math('''<ml:define>
...
add_math('''<ml:define>
    ...
    <ml:real>0.01</ml:real>
</ml:define>''', left=570, top=top_splits, width=80, height=12, advance=True, dtop=40)

top_splits2 = current_top + 5
add_math('''<ml:define>
    <ml:id xml:space="preserve" subscript="4">HK</ml:id>
    <ml:real>0.01</ml:real>
</ml:define>''', left=30, top=top_splits2, width=80, height=12, advance=True, dtop=45)
...
</ml:define>''', left=140, top=top_f1_spec, width=100, height=95, advance=False)
...
</ml:define>''', left=270, top=top_f1_spec, width=120, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>1</ml:real></ml:apply>
</ml:eval>''', left=420, top=top_f1_spec, width=120, height=95, advance=True, dtop=125)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.1">f</ml:id>
</ml:eval>''', left=180, top=top_f2_1, width=100, height=25, advance=True, dtop=45)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.2">f</ml:id>
</ml:eval>''', left=510, top=top_c2, width=120, height=25, advance=True, dtop=45)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.3">f</ml:id>
</ml:eval>''', left=550, top=top_c3, width=90, height=25, advance=True, dtop=45)
...
</ml:define>''', left=510, top=top_xi, width=100, height=35, advance=True, dtop=55)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.4">f</ml:id>
</ml:eval>''', left=500, top=top_c4, width=120, height=25, advance=True, dtop=45)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.5">f</ml:id>
</ml:eval>''', left=400, top=top_c5, width=100, height=25, advance=True, dtop=45)
...
add_text([
    ...
    "   (Nota de cátedra: El valor de 60 mol/h mencionado en el enunciado corresponde al caso de referencia de paso simple",
    "   sin reciclo con F1=100 mol/h [97*0.6 = 58.2 ~ 60 mol/h]; con reciclo activo, F14 se ajusta exactamente por esta relación)."
], style="Normal", left=30, width=650)

top_f2_6 = current_top + 10
add_math('''<ml:define>
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="2.6">f</ml:id>
</ml:eval>''', left=180, top=top_f2_6, width=120, height=25, advance=True, dtop=45)

# Vector f<2> completo
add_text(["Vector de alimentación total al reactor R1 (f<2>):"], style="Normal", left=30)
top_vec_f2 = current_top + 20
add_math('''<ml:define>
...
</ml:define>''', left=30, top=top_vec_f2, width=100, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>2</ml:real></ml:apply>
</ml:eval>''', left=150, top=top_vec_f2, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_f3, width=150, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>3</ml:real></ml:apply>
</ml:eval>''', left=200, top=top_f3, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_t1, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>4</ml:real></ml:apply>
</ml:eval>''', left=210, top=top_t1, width=120, height=95, advance=False)
...
</ml:define>''', left=350, top=top_t1, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>5</ml:real></ml:apply>
</ml:eval>''', left=530, top=top_t1, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_s1, width=140, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>6</ml:real></ml:apply>
</ml:eval>''', left=190, top=top_s1, width=120, height=95, advance=False)
...
</ml:define>''', left=330, top=top_s1, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>7</ml:real></ml:apply>
</ml:eval>''', left=510, top=top_s1, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_t2, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>8</ml:real></ml:apply>
</ml:eval>''', left=210, top=top_t2, width=120, height=95, advance=False)
...
</ml:define>''', left=350, top=top_t2, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>9</ml:real></ml:apply>
</ml:eval>''', left=530, top=top_t2, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_s2, width=140, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>15</ml:real></ml:apply>
</ml:eval>''', left=190, top=top_s2, width=120, height=95, advance=False)
...
</ml:define>''', left=330, top=top_s2, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>16</ml:real></ml:apply>
</ml:eval>''', left=510, top=top_s2, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=340, top=top_f14, width=120, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>14</ml:real></ml:apply>
</ml:eval>''', left=480, top=top_f14, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_t3, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>11</ml:real></ml:apply>
</ml:eval>''', left=210, top=top_t3, width=120, height=95, advance=False)
...
</ml:define>''', left=350, top=top_t3, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>10</ml:real></ml:apply>
</ml:eval>''', left=530, top=top_t3, width=120, height=95, advance=True, dtop=130)
...
</ml:define>''', left=30, top=top_t4, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>13</ml:real></ml:apply>
</ml:eval>''', left=210, top=top_t4, width=120, height=95, advance=False)
...
</ml:define>''', left=350, top=top_t4, width=160, height=95, advance=False)

add_math('''<ml:eval>
    <ml:apply><ml:matcol /><ml:id xml:space="preserve">f</ml:id><ml:real>12</ml:real></ml:apply>
</ml:eval>''', left=530, top=top_t4, width=120, height=95, advance=True, dtop=130)
...
top_eval_f = current_top + 20
add_math('''<ml:eval>
    <ml:id xml:space="preserve">f</ml:id>
</ml:eval>''', left=30, top=top_eval_f, width=600, height=95, advance=True, dtop=135)

# Vector unitario u (6x1) para sumas matriciales elegantes F = u^T * f y W = u^T * w
add_text(["B) Vector unitario u (6x1) y Caudales molares totales por corriente F [mol/h] (F := u^T * f):"], style="Normal", left=30)
top_f_tot_build = current_top + 15
...
</ml:define>''', left=30, top=top_f_tot_build, width=80, height=95, advance=False)

add_math('''<ml:define>
    <ml:id xml:space="preserve">F</ml:id>
    <ml:apply>
        <ml:mult />
        <ml:apply><ml:transpose /><ml:id xml:space="preserve">u</ml:id></ml:apply>
        <ml:id xml:space="preserve">f</ml:id>
    </ml:apply>
</ml:define>''', left=140, top=top_f_tot_build, width=120, height=25, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve">F</ml:id>
</ml:eval>''', left=280, top=top_f_tot_build, width=370, height=25, advance=True, dtop=130)

# Matriz de fracciones molares x (6x16)
add_text(["C) Matriz de fracciones molares x (adimensional):"], style="Normal", left=30)
top_x_build = current_top + 15
...
</ml:eval>''', left=230, top=top_w_build, width=410, height=95, advance=True, dtop=135)

# Vector de caudales másicos totales W (1x16) [g/h] (W := u^T * w)
add_text(["E) Vector de caudales másicos totales por corriente W [g/h] (W := u^T * w):"], style="Normal", left=30)
top_W_tot_build = current_top + 15
...
</ml:define>''', left=30, top=top_W_tot_build, width=120, height=25, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve">W</ml:id>
</ml:eval>''', left=170, top=top_W_tot_build, width=470, height=25, advance=True, dtop=50)

# Composiciones en porcentaje en peso y (%p)
add_text(["F) Matriz de composiciones en porcentaje en peso y [%p]:"], style="Normal", left=30)
top_y_build = current_top + 15
...
</ml:eval>''', left=250, top=top_y_build, width=390, height=95, advance=True, dtop=135)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="out">W</ml:id>
</ml:eval>''', left=520, top=top_consist, width=120, height=25, advance=True, dtop=45)

# Delta_W := W_in - W_out
top_delta_w = current_top + 10
add_math('''<ml:define>
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve">Error</ml:id>
</ml:eval>''', left=490, top=top_delta_w, width=120, height=25, advance=True, dtop=50)

add_text([
    "CONCLUSIÓN DEL BALANCE GLOBAL:",
    "Como se observa numéricamente, Delta W = 0.000000 g/h y Error = 0.000000 %, lo que demuestra matemáticamente",
    "la consistencia estricta de todos los balances de materia por unidad y globales formulados en el sistema."
], style="Normal", left=30, width=650)
...
top_tbl_b = current_top + 20
# Precomputed exact values for beta1 in [0.01, 0.05, 0.10, 0.20, 0.30, 0.50, 0.75, 1.00]
tbl_b_vals = [
    ...
    [0.75, 91.98,  206.99, 99.62],
    [1.00, 93.70,  158.20, 99.59]
]
# Serialize in COLUMN-MAJOR order (Mathcad XML requirement)
tbl_b_xml = "".join(f"<ml:real>{tbl_b_vals[r][c]}</ml:real>" for c in range(4) for r in range(8))
...
</ml:define>''', left=30, top=top_tbl_b, width=320, height=140, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="B">Tabla</ml:id>
</ml:eval>''', left=380, top=top_tbl_b, width=250, height=140, advance=True, dtop=165)
...
tbl_c_xml = "".join(f"<ml:real>{tbl_c_vals[r][c]}</ml:real>" for c in range(4) for r in range(8))
...
</ml:define>''', left=30, top=top_tbl_c, width=320, height=140, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="C">Tabla</ml:id>
</ml:eval>''', left=380, top=top_tbl_c, width=250, height=140, advance=True, dtop=165)
...
add_text([
    ...
    "   - La pureza de etanol en F13 se mantiene extraordinariamente alta (>99.5 % mol) para todo el rango de purga.",
...
top_tbl_d = current_top + 20
# beta2 in [1.00, 0.50, 0.20, 0.10, 0.05, 0.01]
tbl_d_vals = [
    [1.00, 242.50, 217.89, 19.65,  0.0, -0.55],
    [0.50, 133.65, 108.95, 19.75, 44.9, -0.46],
    [0.20,  68.35,  43.58, 19.81, 71.8, -0.41],
    [0.10,  46.58,  21.79, 19.83, 80.8, -0.39],
    [0.05,  35.69,  10.89, 19.84, 85.3, -0.39],
    [0.01,  26.98,   2.18, 19.85, 88.9, -0.38]
]
# Serialize in COLUMN-MAJOR order (6 rows x 6 cols)
tbl_d_xml = "".join(f"<ml:real>{tbl_d_vals[r][c]}</ml:real>" for c in range(6) for r in range(6))
...
</ml:define>''', left=30, top=top_tbl_d, width=380, height=120, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="D">Tabla</ml:id>
</ml:eval>''', left=430, top=top_tbl_d, width=210, height=120, advance=True, dtop=145)
...
add_text([
    ...
    "     ese etanol no se pierde en el efluente sino que vuelve al lazo, aumentando la producción de F13 de 19.65 a 19.84 mol/h.",
    "",
...
add_text([
    ...
    "  * Precios de mercado:",
    "    - Etanol industrial:             9.30 u$s / Gal  (densidad = 0.789 g/cm3 ==> 3.11382 u$s / kg)",
    "    - Etileno (cracking):             0.46 u$s / Lb   (1 Lb = 0.45359 kg  ==> 1.01413 u$s / kg)",
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="Etil">Costo</ml:id>
</ml:eval>''', left=510, top=top_eco_calc, width=110, height=25, advance=True, dtop=45)

top_eco_calc2 = current_top + 10
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="Efl">Costo</ml:id>
</ml:eval>''', left=560, top=top_eco_calc2, width=100, height=25, advance=True, dtop=45)

# Potencial Económico PE
top_pe = current_top + 10
add_math('''<ml:define>
...
</ml:define>''', left=30, top=top_pe, width=230, height=25, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve">PE</ml:id>
</ml:eval>''', left=290, top=top_pe, width=110, height=25, advance=False)
...
</ml:define>''', left=430, top=top_pe, width=120, height=25, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="anual">PE</ml:id>
</ml:eval>''', left=570, top=top_pe, width=120, height=25, advance=True, dtop=50)
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="T1">Cost</ml:id>
</ml:eval>''', left=560, top=top_cost_eq, width=100, height=30, advance=True, dtop=50)

top_cost_eq2 = current_top + 10
# Cost_T2: F4/1000
add_math('''<ml:define>
...
add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="T3">Cost</ml:id>
</ml:eval>''', left=520, top=top_cost_eq2, width=100, height=30, advance=True, dtop=50)

top_cost_eq3 = current_top + 10
# Cost_T4: F10/1000
add_math('''<ml:define>
...
</ml:define>''', left=30, top=top_cost_eq3, width=220, height=30, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="T4">Cost</ml:id>
</ml:eval>''', left=280, top=top_cost_eq3, width=100, height=30, advance=True, dtop=45)

top_cost_tot = current_top + 10
...
</ml:define>''', left=30, top=top_cost_tot, width=230, height=30, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="tot">Cost</ml:id>
</ml:eval>''', left=280, top=top_cost_tot, width=120, height=30, advance=True, dtop=50)
...
top_tbl_f = current_top + 45
# Exact precomputed values for beta1 in [0.01, 0.05, 0.10, 0.20, 0.30, 0.50, 0.75, 1.00]
tbl_f_vals = [
    [0.01,  7.88,  63060.77, 27731.00],
    [0.05,  3.51,  28109.92, 27531.47],
    [0.10,  1.27,  10184.10, 28419.14],
    [0.20, -0.55,  -4377.99, 31016.95],
    [0.30, -1.33, -10678.95, 33805.35],
    [0.50, -2.05, -16436.94, 39282.86],
    [0.75, -2.45, -19596.55, 45802.16],
    [1.00, -2.66, -21253.93, 52026.45]
]
# Serialize in COLUMN-MAJOR order
tbl_f_xml = "".join(f"<ml:real>{tbl_f_vals[r][c]}</ml:real>" for c in range(4) for r in range(8))
...
</ml:define>''', left=30, top=top_tbl_f, width=320, height=140, advance=False)

add_math('''<ml:eval>
    <ml:id xml:space="preserve" subscript="F">Tabla</ml:id>
</ml:eval>''', left=380, top=top_tbl_f, width=250, height=140, advance=True, dtop=165)
...
add_text([
    ...
    "     es máximo (produciendo hasta 79.2 mol/h de etanol puro) y el PE es altamente positivo (+63,061 u$s/año a beta_1 = 0.01).",
    ...
    "   - A beta_1 = 1.00 (sin reciclo de gas), las pérdidas operativas ascienden a -21,254 u$s/año.",
    "",
    ...
    "   - A purgas altas (beta_1 -> 1.0) el costo de capital de las torres pequeñas aumenta en términos relativos",
    "     debido a la penalización por pérdida de economía de escala en las correlaciones logarítmicas.",
    "",
