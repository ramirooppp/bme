$filePath = "c:\Users\nahue\Desktop\segundo cuatrimestre\calendario.html"
...
"@
$text = $text.Replace($headerOld.Trim(), $headerNew.Trim())
...
$toastsAndModals = @"
    ...
    <!-- Main Content -->
"@
$text = $text.Replace("<!-- Main Content -->", $toastsAndModals)
...
"@
$text = $text.Replace($bottomOld.Trim(), $bottomNew.Trim())
...
"@
    ...
    "    function saveProgress(data) {`r`n        localStorage.setItem('cal-progress', JSON.stringify(data));`r`n    }" = @"
    function saveProgress(data) {
        localStorage.setItem('cal-progress', JSON.stringify(data));
    }

    // Feature: Streaks & XP Storage
    function getStreakData() { return JSON.parse(localStorage.getItem('streak-data')) || { currentStreak: 0, longestStreak: 0, lastStudyDate: null }; }
    function saveStreakData(d) { localStorage.setItem('streak-data', JSON.stringify(d)); }
    function getXpData() { return JSON.parse(localStorage.getItem('xp-data')) || { totalXP: 0 }; }
    function saveXpData(d) { localStorage.setItem('xp-data', JSON.stringify(d)); }
    function getBadgesData() { return JSON.parse(localStorage.getItem('badges-data')) || {}; }
    function saveBadgesData(d) { localStorage.setItem('badges-data', JSON.stringify(d)); }
    function getTrackerData(dateStr) { return JSON.parse(localStorage.getItem('tracker-' + dateStr)) || {}; }
    ...
    function checkStreak() {
        ...
        if (studiedDates.length === 0) {
            ...
            if (studiedToday) current++;
                ...
                if (!studiedDates.includes(formatDate(yesterday))) current = 0;
            }
...
foreach ($key in $jsMap.Keys) {
    $text = $text.Replace($key, $jsMap[$key])
}

# The HTML event rendering block:
$eventOld = @"
...
"@
$text = $text.Replace($eventOld.Trim(), $eventNew.Trim())

$appendOld = "col.appendChild(el);"
$appendNew = @"
            col.appendChild(el);
...
"@
$text = $text.Replace($appendOld, $appendNew)

$sidebarOld = "// --- 5. SIDEBAR LOGIC ---"
$sidebarNew = @"
        // --- 4.5 QUICK LOG MODAL ---
        ...
        // --- 5. SIDEBAR LOGIC ---
"@
$text = $text.Replace($sidebarOld, $sidebarNew)

$progOld = "saveProgress(progressData);"
$progNew = @"
...
"@
$text = $text.Replace($progOld, $progNew)

$pomoOld = "pomodorosToday++;"
$pomoNew = @"
                            pomodorosToday++;
...
"@
$text = $text.Replace($pomoOld, $pomoNew)
...
$statsNew = @"
        ...
        function updateStats() {
            ...
            let sb = loadStudyBlocks(currentWeekStart);
            sb.forEach(b => { planHrs += (timeToDecimal(b.end) - timeToDecimal(b.start)); });
            FIXED_CLASSES.forEach(c => { classHrs += (timeToDecimal(c.end) - timeToDecimal(c.start)); });
...
"@
# Replace using regex instead of exact string to avoid template literal issues in PowerShell parsing
$text = $text -replace '(?s)function updateStats\(\) \{.*?(?=\r?\n        function updateCurrentTimeLine)', $statsNew
...
$initNew = @"
        ...
        renderDashboard();
"@
$text = $text.Replace($initOld.Trim(), $initNew.Trim())
